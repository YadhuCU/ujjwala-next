...
import {
  TrendingUp, TrendingDown, DollarSign, ShoppingCart, Wallet, Users,
  Package, AlertTriangle, Plus, ArrowUpRight, IndianRupee, BarChart3,
} from "lucide-react";
import { dashboardOptions } from "@/lib/query-options";

const COLORS = [
  "hsl(221, 83%, 53%)", "hsl(160, 60%, 45%)", "hsl(280, 65%, 60%)",
  "hsl(38, 92%, 50%)", "hsl(350, 80%, 55%)", "hsl(190, 70%, 50%)",
  "hsl(45, 85%, 55%)", "hsl(320, 70%, 55%)",
];
...
interface DashboardData {
  ...
  kpis: {
    ...
    totalCollections: number; totalQtySold: number; customerCount: number;
    todayRevenue: number; comSaleCount: number; domSaleCount: number;
  };
  dailyTrend: {
    date: string; revenue: number; cost: number; expense: number;
    profit: number; collections: number; comSales: number; domSales: number;
  }[];
  productBreakdown: { name: string; revenue: number; qty: number; fill: string }[];
  lowStock: { id: number; batchNo: string; productName: string; quantity: number }[];
  recentTxns: { id: number; trNo: string | null; customer: string; product: string; amount: number; date: string; type: string }[];
}
...
export default function DashboardPage() {
  ...
  const fromStr = dateRange?.from ? dateRange.from.toISOString().split("T")[0] : undefined;
  const toStr = dateRange?.to ? dateRange.to.toISOString().split("T")[0] : undefined;
  ...
  const { kpis, dailyTrend, productBreakdown, lowStock, recentTxns, role } = data;
  const isStaff = role === "staff";
  const profitMargin = kpis.totalRevenue > 0 ? ((kpis.totalProfit / kpis.totalRevenue) * 100).toFixed(1) : "0";
  ...
  const kpiCards = [
    ...
    },
    {
      ...
      gradient: kpis.totalProfit >= 0 ? "from-emerald-500 to-teal-600" : "from-red-500 to-rose-600",
      shadow: kpis.totalProfit >= 0 ? "shadow-emerald-500/20" : "shadow-red-500/20",
      staffVisible: false,
    },
    {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            {isStaff ? "Your personal sales & collections overview" : "Analytics overview for your gas agency"}
          </p>
        </div>
      ...
      {/* KPI Cards */}
      <div className={`grid gap-4 sm:grid-cols-2 ${isStaff ? 'lg:grid-cols-2' : 'lg:grid-cols-4'}`}>
      ...
      <div className={`grid gap-6 ${isStaff ? '' : 'lg:grid-cols-7'}`}>
        ...
        <Card className={isStaff ? '' : 'lg:col-span-4'}>
          <CardHeader>
            ...
            <CardDescription>
              {isStaff ? "Daily revenue from your sales" : "Daily revenue, profit, and expenses over the selected period"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <ChartContainer config={trendConfig} className="h-[300px] w-full">
              <AreaChart data={dailyTrend} margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                ...
                {!isStaff && <Area type="monotone" dataKey="profit" stroke="var(--color-profit)" fill="url(#fillProfit)" strokeWidth={2} />}
                {!isStaff && <Area type="monotone" dataKey="expense" stroke="var(--color-expense)" fill="none" strokeWidth={2} strokeDasharray="4 4" />}
              </AreaChart>
            </ChartContainer>
          </CardContent>
        </Card>
        ...
        <Card className="lg:col-span-3">
          ...
          <CardContent>
            <ChartContainer config={salesTypeConfig} className="h-[300px] w-full">
              <BarChart data={dailyTrend} margin={{ top: 5, right: 10, left: 10, bottom: 0 }}>
                ...
                <ChartLegend content={<ChartLegendContent />} />
                <Bar dataKey="comSales" stackId="s" fill="var(--color-comSales)" radius={[0, 0, 0, 0]} />
                <Bar dataKey="domSales" stackId="s" fill="var(--color-domSales)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ChartContainer>
          </CardContent>
        </Card>
          ...
          <CardContent>
            {productBreakdown.length === 0 ? (
              ...
              <ChartContainer config={productConfig} className="mx-auto h-[280px] w-full">
                <PieChart>
                  <ChartTooltip
                    content={<ChartTooltipContent hideLabel formatter={(value) => `₹${Number(value).toLocaleString("en-IN")}`} />}
                  />
                  <Pie
                    data={productBreakdown.map((p, i) => ({ ...p, fill: COLORS[i % COLORS.length] }))}
                    dataKey="revenue"
                    nameKey="name"
                    innerRadius={60}
                    outerRadius={100}
                  ...
                  {lowStock.map((s) => (
                    <TableRow key={s.id}>
                      ...
                      <TableCell className="text-right">
                        <Badge variant={s.quantity <= 3 ? "destructive" : "secondary"} className="tabular-nums">
                          {s.quantity}
                        </Badge>
                      </TableCell>
                    </TableRow>
            ...
            {recentTxns.length === 0 ? (
              ...
              <Table>
                ...
                <TableBody>
                  {recentTxns.map((t) => (
                    <TableRow key={t.id}>
                      <TableCell className="font-medium font-mono text-xs">{t.trNo}</TableCell>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-3">
            <Button asChild>
              <Link href="/sales/add"><Plus className="w-4 h-4 mr-2" />New Commercial Sale</Link>
            </Button>
            <Button asChild variant="secondary">
              <Link href="/dom-sales/add"><Plus className="w-4 h-4 mr-2" />New Domestic Sale</Link>
            </Button>
            <Button asChild variant="outline">
              <Link href="/expenses/add"><Plus className="w-4 h-4 mr-2" />Add Expense</Link>
            </Button>
            <Button asChild variant="outline">
              <Link href="/stock"><Package className="w-4 h-4 mr-2" />Manage Stock</Link>
            </Button>
          </div>
        </CardContent>
      </Card>
