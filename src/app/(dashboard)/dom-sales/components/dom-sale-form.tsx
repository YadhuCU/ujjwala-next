...
export function DomSaleForm({
  ...
  const form = useForm<DomSaleFormValues>({
    resolver: zodResolver(domSaleSchema) as Resolver<DomSaleFormValues>,
  ...
  const handleSubmit = (values: DomSaleFormValues) => {
  ...
  return (
    <Card className="container mr-auto">
      <CardHeader>
        <CardTitle>{isEditMode ? "Domestic Sale Details" : "New Domestic Sale"}</CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          <form
