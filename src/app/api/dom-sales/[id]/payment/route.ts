...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const newPaid = Number(domSale.paidAmount) + amount;
  if (newPaid > Number(domSale.totalAmount)) {
  ...
  const [updatedSale, collection] = await prisma.$transaction([
    prisma.domSale.update({
      where: { id: parseInt(id) },
      data: { paidAmount: newPaid },
    }),
    prisma.collection.create({
      data: {
        customerId: domSale.customerId,
        domSaleId: parseInt(id),
        amount,
